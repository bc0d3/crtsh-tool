...
func main() {
	...
	timeout := flag.Int("t", 10, "Timeout en segundos para las peticiones")
	...
	if *version {
		fmt.Printf("crtsh-finder version %s\n", Version)
		return
	}
	...
	if *domain == "" {
		...
		fmt.Println("Uso: crtsh-finder -d dominio.com [-o output.txt] [-t timeout] [-f format]")
		os.Exit(1)
	}
	...
	// Iniciar búsqueda
	if !*silent {
		fmt.Printf("Buscando subdominios 'crt.sh' para: %s\n", *domain)
	}
